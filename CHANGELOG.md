...
### 🐛 Bug 修复
1. **书架详情显示空白** - 修复 dynasty 字段引用，兼容新模型 author 格式
2. **添加到小集功能** - 新增书架页面"添加到小集"按钮和选择器
3. **预设标签简化** - 只保留 唐诗/宋词/故事/童话 四个标签
4. **收藏界面刷新** - 修复 Obx 监听，收藏后实时显示
5. **小集编辑功能** - 实现编辑对话框，可修改名称和描述
6. **主题颜色按钮** - 修复10种颜色名称显示
...
### 🏗️ 架构重构
- **全新数据架构**：从层级分组升级为标签+歌单模式
- **数据驱动初始化**：App首次启动从JSON自动加载内置诗词
- **新数据库Schema**：5张表设计（poems/tags/poem_tags/collections/collection_poems）

### ✨ 新增功能
- **标签系统**：支持多维度分类（古诗/宋词/元曲/童话/必背等）
- **小集（歌单）功能**：创建个人收藏集，支持拖拽排序
- **Cherry Studio风格书架页**：全新设计的标签筛选+搜索体验
- **播放上下文**：支持按标签/小集/收藏生成播放队列

### 🔧 技术改进
- 新增 PoemService 业务逻辑层
- DatabaseHelper 完整重构，保留旧API兼容层
- 内置7首诗词数据（古诗/宋词/元曲/童话/现代文）
